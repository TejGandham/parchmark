...
"""
    ...
    no_title = factory.LazyFunction(
        lambda: fake.paragraph(nb_sentences=3)
    )
    
    empty = ""
    
    only_title = factory.LazyFunction(
        lambda: f"# {fake.sentence()}"
    )
...
"""
    ...
    title_with_formatting = factory.LazyFunction(
        lambda: f"# **{fake.word()}** with *{fake.word()}*"
    )
...
class AuthTokenFactory:
    ...
    def create_token_data(username: str = None) -> Dict[str, str]:
        ...
        return {
            "sub": username or fake.user_name()
        }
...
class APIResponseFactory(factory.DictFactory):
    ...
    success_message = factory.LazyFunction(
        lambda: {"message": fake.sentence()}
    )
    
    error_response = factory.LazyFunction(
        lambda: {"detail": fake.sentence()}
    )
    
    validation_error = factory.LazyFunction(
        lambda: {
            "detail": "Validation error",
            "errors": [fake.sentence() for _ in range(fake.random_int(1, 3))]
        }
    )
...
class DatabaseTestDataFactory:
    ...
    def create_multiple_users_with_notes(session, num_users: int = 2, notes_per_user: int = 2):
        ...
        for i in range(num_users):
            user, notes = DatabaseTestDataFactory.create_user_with_notes(
                session, notes_per_user
            )
            users_data.append((user, notes))
