...
class TestCreateNoteTransactionError:
    ...
    async def test_create_note_refresh_failure_returns_500(self, mock_user, mock_db_session):
        ...
        Note: If commit succeeds but refresh fails, the data is already persisted.
        The rollback has no effect but is safe to call. This test verifies the
        error handling path, not data rollback.
        """
        # Arrange
        note_data = NoteCreate(title="Test Note", content="# Test Note\n\nContent here.")
        ...
        assert exc_info.value.status_code == status.HTTP_500_INTERNAL_SERVER_ERROR
        assert "Database error" in exc_info.value.detail
        # Verify commit was called before refresh failed
