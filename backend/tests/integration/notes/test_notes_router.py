...
class TestUpdateNoteEndpoint:
    ...
    def test_update_note_content_only(self, client: TestClient, auth_headers, sample_note):
        ...
        update_data = {
            "content": "# New Title From Content\n\nNew content."
        }
...
class TestNotesRouterErrorHandling:
    ...
    def test_invalid_note_ids(self, client: TestClient, auth_headers):
        ...
        for invalid_id in invalid_ids:
            ...
            assert get_response.status_code in [status.HTTP_200_OK, status.HTTP_404_NOT_FOUND, status.HTTP_405_METHOD_NOT_ALLOWED, status.HTTP_422_UNPROCESSABLE_ENTITY]
            
            # Test PUT
            update_data = {"title": "Test"}
            put_response = client.put(f"/api/notes/{invalid_id}", headers=auth_headers, json=update_data)
            assert put_response.status_code in [status.HTTP_404_NOT_FOUND, status.HTTP_405_METHOD_NOT_ALLOWED, status.HTTP_422_UNPROCESSABLE_ENTITY]
            
            # Test DELETE
            delete_response = client.delete(f"/api/notes/{invalid_id}", headers=auth_headers)
            assert delete_response.status_code in [status.HTTP_404_NOT_FOUND, status.HTTP_405_METHOD_NOT_ALLOWED, status.HTTP_422_UNPROCESSABLE_ENTITY]
