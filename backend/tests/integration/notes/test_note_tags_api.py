...
@pytest.mark.asyncio
async def test_get_notes_returns_tags(client: TestClient, auth_headers, sample_note, test_db_session):
    await test_db_session.refresh(sample_note, attribute_names=["tags"])
...
@pytest.mark.asyncio
async def test_update_note_replaces_clears_and_preserves_tags(
...
@pytest.mark.asyncio
async def test_update_note_normalizes_deduplicates_and_sorts_tags(
    client: TestClient, auth_headers, sample_note, test_db_session
):
    await test_db_session.refresh(sample_note, attribute_names=["tags"])
    sample_note.tags = [NoteTag(tag="draft")]
...
@pytest.mark.asyncio
async def test_update_note_adds_and_removes_tags_without_reinserting_kept_tags(
    client: TestClient, auth_headers, sample_note, test_db_session
):
    await test_db_session.refresh(sample_note, attribute_names=["tags"])
...
@pytest.mark.asyncio
async def test_update_note_with_invalid_tags_keeps_existing_tags(
    client: TestClient, auth_headers, sample_note, test_db_session
):
    await test_db_session.refresh(sample_note, attribute_names=["tags"])
...
@pytest.mark.asyncio
async def test_user_cannot_see_another_users_tags(client: TestClient, test_db_session):
