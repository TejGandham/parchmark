...
def multiple_notes(test_db_session: Session, sample_user: User) -> list[Note]:
    ...
    notes_data = [
        ...
        {"id": "note-2", "title": "Second Note", "content": "# Second Note\n\nThis is the second test note."},
        {"id": "note-3", "title": "Third Note", "content": "# Third Note\n\nThis is the third test note."},
    ]

    notes = []
    for note_data in notes_data:
        note = Note(id=note_data["id"], user_id=sample_user.id, title=note_data["title"], content=note_data["content"])
        test_db_session.add(note)
        notes.append(note)
...
class TestDataFactory:
    ...
    def create_user_data(username: str = None, password: str = None) -> dict[str, str]:
        ...
        return {"username": username or f"user_{datetime.now().timestamp()}", "password": password or "testpass123"}
